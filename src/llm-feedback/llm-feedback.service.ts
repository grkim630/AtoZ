...
export class LlmFeedbackService {
  ...
  async ingest(
    ...
    if (!session.llmBehaviorEval) {
      throw new BadRequestException("Session evaluation is required before feedback ingest");
    }
      ...
      ({
        ...
        avgRiskWeight:
          session.actionEvents.length === 0
            ? 0
            : session.actionEvents.reduce((acc, event) => acc + event.riskWeight, 0) /
    ...
    return this.prisma.llmFeedbackDataset.create({
      data: {
        ...
        inputFeaturesJson: inputFeaturesJson as Prisma.InputJsonValue,
        targetLabelJson: targetLabelJson as Prisma.InputJsonValue,
        qualityFlag,
