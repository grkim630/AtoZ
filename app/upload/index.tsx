    ...
    <View style={styles.gaugeContainer}>
      ...
      <View style={styles.gaugeTextContainer}>
        ...
        <View style={styles.gaugeLabelBadge}>
           <Text style={styles.gaugeLabelText}>위험</Text>
        </View>
      </View>
    </View>
  );
...
const ComparisonChart = ({ data }: { data: AnalysisResult['comparisonData'] }) => {
  return (
    <View style={styles.chartContainer}>
        <View style={styles.chartLegend}>
            <View style={styles.legendItem}>
                <View style={[styles.legendDot, { backgroundColor: '#CCDBFF' }]} /> 
                <Text style={styles.legendText}>평균 결과</Text>
            </View>
             <View style={styles.legendItem}>
                <View style={[styles.legendDot, { backgroundColor: Colors.error }]} />
                <Text style={styles.legendText}>분석 결과</Text>
            </View>
        </View>
        ...
        {data.map((item, index) => (
          <View key={index} style={styles.barGroup}>
            <View style={styles.barPair}>
                ...
                {/* User Bar */}
                <View style={styles.barWrapper}>
                     <View style={[styles.bar, { height: item.user, backgroundColor: Colors.error }]} />
                </View>
            </View>
  ...
  <View style={styles.analysisCard}>
    ...
    <View style={styles.analysisSection}>
        <View style={styles.gaugeRow}>
            <SemiCircleGauge score={result.riskScore} />
            <View style={styles.riskInfo}>
                <Text style={styles.riskDescription}>
                    <Text style={{ fontWeight: 'bold', color: Colors.error }}>'{result.riskLevel}'</Text> 수준으로,{'\n'}
                    평균 대비 <Text style={{ fontWeight: 'bold', color: Colors.primary }}>{result.diffFromAvg}%</Text> 높아요.
                </Text>
            </View>
        </View>
    </View>
    ...
    {/* Section 2: Chart */}
    <View style={styles.analysisSection}>
        <ComparisonChart data={result.comparisonData} />
    </View>
    ...
    <Text style={styles.verdictText}>
        해당 파일은 <Text style={{ color: Colors.primary, fontWeight: 'bold' }}>'{result.verdictTitle}'</Text> 피싱으로 보여요.{'\n'}
        아래 대응안을 추천해요.
    </Text>
     ...
     <View style={styles.actionCard}>
           ...
           <View style={styles.actionContent}>
                <Text style={styles.actionTag}>사칭형 피싱 대응</Text>
...
export default function UploadScreen() {
  ...
  const processFile = async (file: any) => {
      ...
      // 3. Simulate Analysis
      try {
          const result = await analyzeFile(file.uri);
