...
## `extracted-text-py/app` 연동 방법(선택)

`extracted-text-py`는 `EVALUATION_API_URL`이 설정되어 있으면, clean text를 이 프로젝트의 `/evaluate`로 보내서
`evalLabel`, `evalRiskScore`를 결과에 합쳐줍니다.

1) `evaluation-py`에서 API 서버를 실행합니다.

```powershell
.\.venv\Scripts\python.exe -m uvicorn api:app --host 127.0.0.1 --port 8010
```

2) `extracted-text-py/.env`에 아래를 추가합니다.

```env
EVALUATION_API_URL=http://127.0.0.1:8010
```

3) `extracted-text-py`를 실행하면, 파이프라인 중간에 모델 추론 결과가 자동으로 합쳐집니다.
