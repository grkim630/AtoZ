...
export class ExtractedTextService {
  ...
  private readonly pythonInlineCode = [
    'import json, os, sys',
    'from src.main import analyze_pipeline',
    'from src.services_stt import stt_transcribe',
    'from src.services_ocr import ocr_extract_text',
  ...
  private getPythonProjectRoot() {
    return path.resolve(this.getProjectRoot(), '..', 'python', 'extraction');
  }
  ...
  private getPythonWorkingDir() {
    return this.getPythonProjectRoot();
  }
  ...
  private async runExtractionEngine(
    ...
    // Python OpenAI SDK uses /v1 internally.
    // If OPENAI_BASE_URL is set to ".../v1/responses", it becomes /responses/responses.
  ...
  async listByUser(userId: string, filter?: { uploadedFileId?: string }) {
    return this.prisma.extractedText.findMany({
      where: {
        ...(filter?.uploadedFileId
          ? { uploadedFileId: filter.uploadedFileId }
          : {}),
        uploadedFile: { userId },
      },
