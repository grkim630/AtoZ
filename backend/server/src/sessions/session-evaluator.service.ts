...
export class SessionEvaluatorService {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return this.mockEvaluate(
          input,
          `mock-evaluator:http-${response.status}`,
        );
      }
  ...
  private mockEvaluate(
    ...
    const behaviorBase = computeBehaviorScore(
      input.actionEvents.map((event) => event.riskWeight),
    );
    ...
    const overallScore = Math.round(
      categories.reduce((acc, category) => acc + category.score, 0) /
        categories.length,
    );
    ...
    try {
      ...
      const overallScore =
        typeof parsed.overallScore === 'number'
          ...
          : Math.round(
              categories.reduce((acc, item) => acc + item.score, 0) /
                categories.length,
            );
      const confidence =
      ...
      return {
        ...
        educationMessage: {
          summary:
            parsed.educationMessage?.summary ?? '추가 학습이 필요합니다.',
