...
export function computeConfidence(eventCount: number, messageCount: number) {
  const raw =
    0.4 + Math.min(0.3, eventCount * 0.05) + Math.min(0.3, messageCount * 0.03);
...
export function computeFinalCategoryScore(
  ...
  const penalized =
    confidence < CONFIDENCE_PENALTY_THRESHOLD
      ? merged - CONFIDENCE_LABEL_PENALTY
      : merged;
...
export function computeOverallScore(
  ...
  return round(
    scores.reduce(
      (acc, score) =>
        acc + score.finalScore * CATEGORY_WEIGHTS[score.categoryCode],
      0,
    ),
  );
}
